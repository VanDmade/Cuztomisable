...
- **Dead code found:** `verification()` and `unsubscribe()` exist on the controller but have no route in `routes/api.php` at all - confirmed via a full read of the routes file. Decide whether these come back (routes/api.php fix) or get dropped during migration, rather than silently porting unreachable code forward.
