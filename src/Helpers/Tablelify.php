...
class Tablelify
{
    ...
    public static function run(mixed $query, array $parameters, array $searchColumns = []): JsonResponse
    {
        ...
        if ($parameters['size'] === 'all') {
            $maxAll = (int) config('tablelify.max_size', 100);
            $totalResults = !is_null($parameters['search']) ? $filtered : $total;
            if ($maxAll > 0 && $totalResults > $maxAll) {
        ...
        if (is_null($columns) || empty($columns)) {
            if (is_array($defaultColumns) && !empty($defaultColumns)) {
                foreach ($defaultColumns as $column => $dir) {
                    $query->orderBy($column, $dir ?? config('tablelify.default.order_direction', 'asc'));
                }
                return $query;
            }
     ...
     * @return array The list of data that is generated by the Tablelify class
    \**************************************************************************/
    public static function execute(mixed $query, mixed $size, mixed $page): array
    {
